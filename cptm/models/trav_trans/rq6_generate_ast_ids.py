...
from utils import file_tqdm, rq6_separate_dps
...
def external(file_path, suffix, n_ctx, overlap):
    ...
    with open(file_path, "r") as f, open(outfile, "w") as fout:
        for line in file_tqdm(f):
            ...
            asts = rq6_separate_dps(dp, n_ctx, overlap)
            for ast, _ in asts:
                ids = {}
                if len(ast) > 1:
                    if "all" in {"leaf", "all"}:
...
def main():
    ...
    with open(args.ast_fp, "r") as f, open(args.out_fp, "w") as fout:
        for line in file_tqdm(f):
            dp = json.loads(line.strip())
            asts = rq6_separate_dps(dp, args.n_ctx)
            for ast, _ in asts:
                ids = {}
                if len(ast) > 1:
