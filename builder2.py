...
from model2 import Block, TryBlock, Link, CFG, FuncBlock
...
class CFGBuilder(ast.NodeVisitor):
    ...
    def build_from_src(self, name, src):
        ...
        cfg = self.build(name,tree, asynchr= False)
        cfg.lineno = 1
...
#        classcfg.end_lineno = node.end_lineno  # type: ignore  #TODO something with ignore "'ClassDef' object has no attribute 'end_lineno'"
    ...
    def create_jsondata(self, block,visited, mylist=[]):
        if block in visited:
            return mylist
        visited.add(block)
        ...
        dict = {"id": block.id, "text": block.statements, "children": children, "type": type(block)}
        mylist.append(dict)
        modifiedlist = mylist
        for exit in block.exits:
    ...
    def main(self,path, name):
        cfg = self.build_from_file(name =name,filepath= path)
